...
Claude Code owns **three** files, not one. Beyond `~/.claude/settings.json`, the install flow also calls `resetClaudeAuth()` from `InstallApp.handleInstallDone` whenever any Claude tool (CLI or either extension) survives the install step. `resetClaudeAuth` backs up `~/.claude.json` then writes a fresh `{hasCompletedOnboarding: true}` (so the CLI skips its first-run wizard), and backs up + removes `~/.claude/.credentials.json` (so the CLI can't reuse stale session auth that would conflict with the gateway API key in settings.json). It runs silently — no Phase, no Step — and is best-effort. This applies even on the "Skip configuration" auth path: the user picked a Claude tool, so its prior auth state still needs to be reset for the new install to work; only the settings.json write is skipped. The settings.json backup itself still happens at configure time via `configureClaudeCode` / `backupOnly`.

The install flow's "Skip configuration" auth choice routes through `backupOnly(tool)` instead of the per-agent `configure*` functions: it runs the same `ensureBackup` logic for the agent's main config file (so any existing live config is snapshotted to `*.backup` exactly once) and then exits without writing CoDev's own config. `Configure` accepts `creds: Credentials | null`; `null` is the signal to take this backup-only path.
