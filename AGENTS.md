...
1. **Install wiring.** Tools map to CodeGraph `--target` ids via `toolToCodegraphTarget` (the three CLI agents, plus both Claude Code *extension* variants → `claude`; Continue → none). The work is split in two:
   - **CLI install** (`ensureCodegraphInstalled` = `npm i -g @colbymchenry/codegraph`, always) runs *before* finalize: in **install mode** it's an extra row in the "Installing packages" `TaskList` (so it installs in parallel with the agents — `src/components/Install.tsx`, keyed `CODEGRAPH_TASK_KEY`); in **config mode** (no Install step) it's kicked off right after login in `handleLoginDone` and stashed in `codegraphInstallRef`. The `CODEGRAPH_TASK_KEY` sentinel is **not** a `Tool` — `handleInstallDone` splits it out of the survivor set (Configure/shims would choke on it) and excludes it from the all-failed fail-stop.
   - **MCP wiring** (`setupCodegraph` → `runCodegraphInstall` = `codegraph install --target <csv> --location global --yes`) runs in `runFinalizeSideEffects`, which first `await`s `codegraphInstallRef` (no-op in install mode). `setupCodegraph` assumes the CLI is already installed — it only wires.
